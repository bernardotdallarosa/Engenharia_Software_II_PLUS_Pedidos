# Le plus-infra/.env e corre terraform apply com os TF_VAR esperados pelo Makefile UNIX.
...
try {
    if (-not (Test-Path ".env")) {
        Write-Host '[make] ERRO: .env nao encontrado na raiz do plus-infra.'
        exit 1
    }
    ...
    Write-Host '[make] Sincronizando estado com S3/RDS ja existentes no Ministack (se aplicavel)...'
