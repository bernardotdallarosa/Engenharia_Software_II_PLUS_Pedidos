# Aguarda o endpoint de health do Ministack ate servicos estarem registados no JSON.
$ErrorActionPreference = "Continue"
$url = if ($env:MINISTACK_URL) { $env:MINISTACK_URL } else { "http://localhost:4566/_localstack/health" }

Write-Host '[make] Aguardando Ministack ficar disponivel...'
...
Write-Host '[make] ERRO: Ministack nao ficou disponivel a tempo.'
exit 1
